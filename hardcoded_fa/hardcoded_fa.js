/*	@author 		ntoombs19 & Hardcode
 *	name 			FA Filter
 *	version 		3.1
 *	description		Maakt het gebruik van FA filter makkelijker
 */
...
function customSendUnitsFromReport(link, target_village, report_id, button) {
  ...
  $.post(Accountmanager.send_units_link_from_report, data, function (data) {
    if (data.error) {
      UI.ErrorMessage(data.error);
      button.closest("tr").show().delay(250);
    }
    else {
      setLocalStorageRow(button);
...
function runScript(){
  if(!pagesLoaded){
    moreRowsTip();
    removeFirstPage();
    setTimeout(function(){showAllRows()}, 1);
  }
  else{
    applyFilters();
  }
...
function loadLanguage(lang){
  ...
  $.getScript('https://www.iwantwin.nl/scripts/tribalwars/hardcoded_fa/'+lang+'.js',function(){
    $('#settingsDiv').remove();
